classdef DCLContourTracking<handle
...
% older than R2018a, and without some toolboxes.
%
%
% Future implementations/changes:
%       - Expand the documentation
%       - There was a substantial decrease in performance after a change 
%       in the parfor processing loop: this should be improved to bring the
%       processing times back to smaller values
...
%       - 25/07/2020:
%           . Added a field in the saved structure with some info about the
%           processing (date/time, user) to be used in other functions to
%           determine whether some processing was performed on the latest
...
%       - 06/06/2020:
%           . Movie slider had a lot of inertia, and also produced some
...
methods
     % Constructor
        function obj = DCLContourTracking
            %% Parameters
    ...
    % Basename
    obj.Handles.BasenameDisplay =  uicontrol('Style','text','String','','FontSize',obj.FontScaling*12,'FontName','Arial','FontWeight','normal',...
        'Units','Normalized','Position',[0.075 obj.SubMovie.Position(2)+obj.SubMovie.Position(4)+0.005+0.05 obj.SubMovie.Position(3) 0.025],'HorizontalAlignment','left');
 
    
    % Player uicontrols
    obj.Handles.PlayPauseButton = uicontrol('Style','pushbutton','String','Play','FontSize',obj.FontScaling*16,'FontName','Arial','FontWeight','bold',...
    ...
    obj.Handles.IncreaseRateButton = uicontrol('Style','pushbutton','String','Faster','FontSize',obj.FontScaling*16,'FontName','Arial','FontWeight','bold',...
        'Callback',{@(~,~)obj.IncreaseFrameRate},'Units','Normalized','Position',[ButtonsXStart+0.07*2 0.3 0.07 0.05]);
    obj.Handles.CurrentTimeEdit = uicontrol('Style','edit','String','0','FontSize',obj.FontScaling*16,'FontName','Arial','FontWeight','bold',...
        'Callback',{@(~,~)obj.EditCurrentTimeCB},'Units','Normalized','Position',[ButtonsXStart+0.07*3 0.3 0.07 0.05]);
    ...
    obj.Handles.CalibrationLine = [];
    obj.Parameters.Calibration.Line = [];
    % CLim
...
methods(Hidden)
    ...
    function obj = LoadFileCB(obj)
        ...
        obj.Handles.BasenameDisplay.String = obj.Basename;
    end
    ...
    function varargout = ProcessFrame(obj,ModeProcessFrame)
        ...
        if obj.Movie.hasFrame,
            ...
            obj.PreTime = obj.Movie.CurrentTime;
            obj.Handles.CurrentTimeEdit.String = num2str(0.01*round(obj.CurrentTime*100));
            ...
            if ~strcmpi(ModeProcessFrame,'Initialize')
                ...
                if ~isempty(obj.Parameters.Calibration.Line)
                    obj.Handles.CalibrationLine = drawline('Position',obj.Parameters.Calibration.Line,'Parent',obj.SubMovie,'Color','c');
                    addlistener(obj.Handles.CalibrationLine,'MovingROI',@(~,~)obj.ReshapeCalibration);
                    obj.Handles.CalibrationDraw.String = 'Delete';
                    obj.Handles.CalibrationDraw.Callback = {@(~,~)obj.DeleteCalibrationCB};
                end
                drawnow
            end
            if SliderUpdate,
    ...
    function SetPlay(obj,varargin)
        if isempty(varargin)
            Force = false;
        end
        if obj.Play && ~Force
            obj.Play = false;
            obj.Handles.PlayPauseButton.String = 'Play';
        else
            obj.Play = true;
            obj.Handles.PlayPauseButton.String = 'Pause';
            tic
    ...
    function EditCurrentTimeCB(obj)
        TempNewTime = str2double(obj.Handles.CurrentTimeEdit.String);
        if TempNewTime>0 && TempNewTime<=obj.Movie.Duration
            obj.SliderLine.XData = [TempNewTime TempNewTime];
            drawnow
        else
            obj.Handles.CurrentTimeEdit.String = num2str(obj.PreTime);
        end
    ...
    function MovingRP(obj)
     ...
     function RPEditCB(obj)
         if str2double(obj.Handles.RP.Edit.String)>=1 && str2double(obj.Handles.RP.Edit.String)<=10,
             ...
             obj.Handles.RP.Edit.Value = num2str(obj.Handles.RP.Line.XData(1));
         end
         obj.UpdateFrame;
     end
     ...
     function DrawCalibrationCB(obj)
         obj.Handles.CalibrationDraw.Callback = [];
         obj.Handles.CalibrationLine = drawline(obj.SubMovie,'Color','c');
         obj.Parameters.Calibration.Line = obj.Handles.CalibrationLine.Position;
         addlistener(obj.Handles.CalibrationLine,'MovingROI',@(~,~)obj.ReshapeCalibration);
         obj.Handles.CalibrationDraw.String = 'Delete';
         obj.Handles.CalibrationDraw.Callback = {@(~,~)obj.DeleteCalibrationCB};
         drawnow
     ...
     function DeleteCalibrationCB(obj)
         delete(obj.Handles.CalibrationLine);
         obj.Handles.CalibrationLine = [];
         obj.Parameters.Calibration.Line = [];
         obj.Handles.CalibrationDraw.String = 'Draw';
         obj.Handles.CalibrationDraw.Callback = {@(~,~)obj.DrawCalibrationCB};
         drawnow
     end
     
     function ReshapeCalibration(obj)
         obj.Parameters.Calibration.Line = obj.Handles.CalibrationLine.Position;
     end
     ...
     function MC1EditCB(obj)
         ...
         if Temp>=1 && rem(Temp,1) == 0,
             ...
             obj.Parameters.StructuringElement.MC1 = strel('disk',Temp);
             if ~obj.Handles.MC1Value.CheckBox.Value == 1,
                 obj.Parameters.MC1Enable = true;
     ...
     function MC2EditCB(obj)
         ...
         if Temp>=1 && rem(Temp,1) == 0,
             ...
             obj.Parameters.StructuringElement.MC2 = strel('disk',Temp);
             if ~obj.Handles.MC2Value.CheckBox.Value == 1,
                 obj.Parameters.MC2Enable = true;
     ...
     function MO1EditCB(obj)
         ...
         if Temp>=1 && rem(Temp,1) == 0,
             ...
             obj.Parameters.StructuringElement.MO1 = strel('disk',Temp);
             strel('disk',Temp)
             if ~obj.Handles.MO1Value.CheckBox.Value == 1,
                 obj.Parameters.MO1Enable = true;
     ...
     function ProcessCB(obj)
        ...
        parfor P = 1 : size(BatchFiles,1)
             ...
             try
                 ...
                 FrameNumEstimate = round(1.1 * CurrObj.Movie.Duration*CurrObj.Movie.FrameRate);   % VideoReader NumberOfFrames was reported to be sometimes inaccurate
...
%                  space to keep everything
                 ...
                 if strcmpi(CurrObj.Parameters.VideoMode,'RGB'),
                     ...
                     if exist(MM_File,'file'),
                         ...
                         TrackingLog.(CurrObj.Parameters.VideoMode).Times = [];
                     end
                 end
                 TrackingLog.(CurrObj.Parameters.VideoMode).Processing.DateTime = datetime;
                 TrackingLog.(CurrObj.Parameters.VideoMode).Processing.User = getenv('username');
